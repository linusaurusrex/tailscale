...
func (e *userspaceEngine) handleLocalPackets(p *packet.ParsedPacket, t *tstun.TUN) filter.Response {
	...
	// TODO: atomic value
	localAddrs := e.localAddrs.Load().(map[packet.IP]bool)
	if localAddrs[p.DstIP] {
		// Packet is destined for the local system, but got
		// misdirected into the tun interface. Send it back.
		// TODO(danderson): decrement TTL, update checksum.
